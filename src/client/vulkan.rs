...
pub(super) fn create_device(instance: &Arc<Instance>, surface: &Arc<Surface<()>>) -> Result<(Arc<Device>, Queues), Box<Error>> {
...
pub(super) fn create_swapchain(surface: &Arc<Surface<()>>, device: &Arc<Device>, queues: &Queues, dimensions: [u32; 2]) -> Result<(Arc<Swapchain<()>>, Vec<Arc<SwapchainImage<()>>>), Box<Error>> {
