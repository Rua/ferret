...
impl<A> AssetCache<A> {
    ...
    pub fn insert<K: Into<String>>(&mut self, key: K, asset: &AssetHandle<A>) -> Option<WeakHandle<A>> {
...
impl<A> AssetStorage<A> {
    ...
    pub fn insert(&mut self, asset: A) -> AssetHandle<A> {
        let id = self.unused_ids
            .pop_front()
            .unwrap_or_else(|| { self.highest_id += 1; self.highest_id });
